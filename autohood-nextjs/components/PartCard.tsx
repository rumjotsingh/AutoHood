...
export default function PartCard({ part }: PartCardProps) {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    addItem({
      id: part._id,
      name: part.name,
      price: part.pricing?.sellingPrice || part.price,
      quantity: 1,
      image: part.images?.[0] || "/placeholder.jpg",
      type: "part",
    });
  };
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden card-hover">
        ...
        <div className="p-4">
          ...
          <p className="text-2xl font-bold text-primary mb-3">
            {formatPrice(part.pricing?.sellingPrice || part.price)}
          </p>

          <div className="flex items-center justify-between text-sm text-gray-600 mb-3">
            <div className="flex items-center space-x-1">
              <Package className="w-4 h-4" />
              <span>{part.condition || "New"}</span>
            </div>
            <span>Stock: {part.stock?.quantity || 0}</span>
          </div>

          <button
            onClick={handleAddToCart}
            disabled={!part.stock?.quantity}
