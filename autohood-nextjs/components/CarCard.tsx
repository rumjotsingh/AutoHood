...
import { Heart, MapPin, Fuel, Gauge, Calendar, Zap } from "lucide-react";
...
export default function CarCard({ car }: CarCardProps) {
  ...
  const inWishlist = hydrated && isInWishlist(car._id);
  ...
  return (
    <div className="group relative bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
      {/* Image Container */}
      <div className="relative h-56 overflow-hidden bg-gray-100">
        <Link href={`/cars/${car._id}`} className="absolute inset-0 block">
        ...
        {/* Wishlist outside <Link> to avoid invalid <button> in <a> (hydration #418) */}
        <button
          type="button"
          onClick={toggleWishlist}
          aria-label={inWishlist ? "Remove from wishlist" : "Add to wishlist"}
          className="absolute top-3 right-3 z-20 bg-white/95 backdrop-blur-sm rounded-full p-2 shadow-md hover:scale-110 active:scale-95 transition-all duration-200 group/heart"
        >
          <Heart
      ...
      <Link href={`/cars/${car._id}`} className="block p-5">
        ...
        <div className="flex items-baseline gap-2 mb-4">
          ...
          {car.pricing?.originalPrice &&
            car.pricing.originalPrice > car.pricing.sellingPrice && (
        ...
        <div className="grid grid-cols-2 gap-2.5 mb-4">
          <div className="flex items-center gap-2 text-sm text-gray-600 bg-gray-50 rounded-lg px-3 py-2.5 hover:bg-gray-100 transition-colors">
            ...
            <span className="font-medium truncate">{km} km</span>
          </div>
          <div className="flex items-center gap-2 text-sm text-gray-600 bg-gray-50 rounded-lg px-3 py-2.5 hover:bg-gray-100 transition-colors">
