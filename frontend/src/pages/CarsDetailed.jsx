...
function CarsDetailed() {
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      const { orderId, amount, currency, keyId, carDetails, demoMode } = res.data;

      // Handle demo mode for high-value transactions
      if (demoMode) {
        toast.info("Demo Mode: Simulating payment for testing", { autoClose: 2000 });
        setTimeout(() => {
          toast.success("Demo payment successful!");
          navigate(`/cars/payment/${id}/success`);
        }, 2000);
        return;
      }
      ...
      const openRazorpay = () => {
        const options = {
          ...
          currency: currency,
          name: "AutoHood",
          description: `Purchase ${carDetails.name} ${carDetails.model}`,
          order_id: orderId,
          handler: async function (response) {
            try {
              ...
              if (verifyRes.data.success) {
                ...
                navigate(`/cars/payment/${id}/cancel`);
              }
            } catch (error) {
      ...
      toast.error(err.response?.data?.message || "Payment session failed. Please try again.");
    }
  };
